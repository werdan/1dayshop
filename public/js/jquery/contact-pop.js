...
ContactPop = {
    ...
    replaceHref : '/order/', // can be array or string of hrefs or nothing if you want to use jQuery selectors (below)
    
    formPhpLocation : '/order/', // relative path to the backend contact form
    
    pathToContactPop : '/', // relative path to the Contact-Pop directory
    
    contactHeadline : 'Оформить заказ',
    ...
    userActionCounter:  0, //count event for single page load
    ...
    attachFormEvents : function() {
        ...
        jQuery( ContactPop.closeButtonSelector, ContactPop.obj.panel).click( function(ev) {
        	ContactPop.userActionCounter++;
        	_gaq.push(['_trackEvent','AJAX Order form','Close','',ContactPop.userActionCounter]);
        ...
        // Resetable values
        jQuery("div.formItem input").resettable();
        jQuery("div.formItem label").hide();
    ...
    showOverlay : function() {
    	ContactPop.userActionCounter++;
    	_gaq.push(['_trackEvent','AJAX Order form','Open','',ContactPop.userActionCounter]);
    ...
    validateForm: function() {
    	//validate form agains default values
    	ContactPop.validationErrors = false;
    	
        jQuery("div.formItem label").hide();
        //Check that not empty and not default text
    	jQuery("div.formItem input").each(function(){
             if ( jQuery(this).val() == jQuery(this).data("original") || jQuery.trim(jQuery(this).val()) == "") {
            	 jQuery("div.formItem label[for='"+this.name+"']").show();
            	 _gaq.push(['_trackEvent','AJAX Order form','Errors (client)',this.name+': ' + jQuery(this).val(),ContactPop.userActionCounter]);
            	 ContactPop.validationErrors = true;
             }
        });
    	//TODO validate email and telephone
    	jQuery("div.formItem input.email").each(function(){
    ...
    submitForm : function() {
    	ContactPop.userActionCounter++;
    	_gaq.push(['_trackEvent','AJAX Order form','Submit','',ContactPop.userActionCounter]);
    	ContactPop.validateForm();
    	if (ContactPop.validationErrors == true) {
        	_gaq.push(['_trackEvent','AJAX Order form','Errors (client)','Rejected',ContactPop.userActionCounter]);
        	return;
    	}
